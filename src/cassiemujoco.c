...
const char *VISSTRING[mjNVISFLAG][3] = {
    {"Convex Hull", "0", "H"},    {"Texture", "1", "X"},
    {"Joint", "0", "J"},          {"Actuator", "0", "U"},
    {"Camera", "0", "Q"},         {"Light", "0", "Z"},
    {"Tendon", "1", "V"},         {"Range Finder", "1", "Y"},
    {"Constraint", "0", "N"},     {"Inertia", "0", "I"},
    {"SCL Inertia", "0", "S"},    {"Perturb Force", "0", "B"},
    {"Perturb Object", "1", "O"}, {"Contact Point", "0", "C"},
    {"Contact Force", "0", "F"},  {"Contact Split", "0", "P"},
    {"Transparent", "0", "T"},    {"Auto Connect", "0", "A"},
    {"Center of Mass", "0", "M"}, {"Select Point", "0", "E"},
    {"Static Body", "1", "D"},    {"Skin", "1", ";"}};
const char *RNDSTRING[mjNRNDFLAG][3] = {
    {"Shadow", "1", "S"},   {"Wireframe", "0", "W"}, {"Reflection", "1", "R"},
    {"Additive", "0", "L"}, {"Skybox", "1", "K"},    {"Fog", "0", "G"},
    {"Haze", "1", "/"},     {"Segment", "0", ","},   {"Id Color", "0", "."}};

// help strings
const char help_content[] = "Alt mouse button\n"
                            "UI right hold\n"
...
const char help_title[] = "Swap left-right\n"
                          "Show UI shortcuts\n"
                          "Expand/collapse all  \n"
                          "Pause\n"
                          "Free camera\n"
...
#undef X
  ...
  // IMU
  mju_copy_fp(c->cassie_out.pelvis.vectorNav.orientation, &c->d->sensordata[16],
              4);
...
double *cassie_sim_dof_damping(cassie_sim_t *c) { return c->m->dof_damping; }
...
void cassie_vis_init_recording(cassie_vis_t *sim, const char *videofile,
  ...
  char ffmpeg_cmd[1000] = "ffmpeg -hide_banner -loglevel error -y -f rawvideo "
                          "-vcodec rawvideo -pix_fmt rgb24 -s ";
...
void mouse_move(GLFWwindow *w, double xpos, double ypos) {
  ...
  int mod_shift = glfwGetKey_fp(w, GLFW_KEY_LEFT_SHIFT) ||
                  glfwGetKey_fp(w, GLFW_KEY_RIGHT_SHIFT);
...
void mouse_button(GLFWwindow *window, int button, int act, int mods) {
  ...
  if (act == GLFW_PRESS && (curr_time - v->lastclicktm < 0.25) &&
    ...
    int selbody = mjv_select_fp(v->m, v->d, &v->opt, aspectratio, relx, rely,
    ...
    if (selmode == 2 || selmode == 3) {
      ...
      if (selbody >= 0) {
        ...
        mju_mulMatTVec_fp(v->pert.localpos, v->d->xmat + 9 * selbody, tmp, 3,
                          3);
...
void key_callback(GLFWwindow *window, int key, int scancode, int action,
  ...
  if (action == GLFW_RELEASE) {
    ...
    case GLFW_KEY_F5: { // toggle fullscreen
      ...
      v->showfullscreen ? glfwMaximizeWindow_fp(window)
                        : glfwRestoreWindow_fp(window);
    } break;
...
float *cassie_vis_draw_depth(cassie_vis_t *v, cassie_sim_t *c, int width,
  ...
  glfwMakeContextCurrent_fp(v->window);
  mjv_updateScene_fp(c->m, c->d, &v->opt, &v->pert, &v->cam, mjCAT_ALL,
                     &v->scn);
...
bool cassie_vis_draw(cassie_vis_t *v, cassie_sim_t *c) {
  ...
  if (v->showhelp) {
    mjr_overlay_fp(mjFONT_NORMAL, mjGRID_TOPLEFT, viewport, help_title,
                   help_content, &v->con);
  }
  if (v->showinfo) {
    char buf[1024];
    ...
    mjr_overlay_fp(mjFONT_NORMAL, mjGRID_BOTTOMLEFT, viewport, str_paused, buf,
                   &v->con);
  }
